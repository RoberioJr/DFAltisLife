...
if ((group player getVariable ["gbank_in_use_by",player]) != player) exitWith {hint localize "STR_ATM_WithdrawInUseG"}; //Check if it's in use.
 ...
 _antdupgang = LIFE_SETTINGS(getNumber,"rj_desativar_saquegang");
 _dono = ((group player) getVariable "gang_owner");
 /* CHECK DO ANTI-DUPE */
if (_antdupgang isEqualTo 1) then {
    //if (_dono != (getPlayerUID player)) exitWith {hint "RJ ANTI-DUPE: Somente o Dono Da Gangue Pode Sacar o Dinheiro!"};
    if !(_dono isEqualTo (getPlayerUID player)) exitWith {hint "RJ ANTI-DUPE: Somente o Dono Da Gangue Pode Sacar o Dinheiro!"};
};
/*-----*/
...
[6] call SOCK_fnc_updatePartialRJ;
