...
life_call_start = compileFinal '
	if(!(playerSide isEqualto INDEPENDENT)) exitWith {hint localize "STR_DC_noaccess"};
	...
	lbClear _playerslb;
	if((count life_med_calls) > 0) then {
		{
		...
		} forEach life_med_calls;
	} else {
		_playerslb lbAdd (localize "STR_DC_nodispatches");
		_playerslb lbSetData [0, "-1"];
	};
	_cfgmsg = getArray(missionConfigFile >> "aapps_calls" >> "med_call_messages");
...
life_call_lbchange = compileFinal '
	...
	if(((count life_med_calls)-1) < _cur) exitwith {
	...
	_playerdata = life_med_calls select _cur;
...
life_call_accept = compileFinal '
	...
	if(((count life_med_calls)-1) < _cur) exitwith {hint localize "STR_DC_reload"};
	_temp = life_med_calls select _cur;
...
life_call_delete = compileFinal '
	...
	if((_playerslb lbData _cur) isEqualTo "-1") exitWith {};
	if(((count life_med_calls)-1) < _cur) exitwith {hint localize "STR_DC_reload"};
	_data = life_med_calls select _cur;
	...
	if(!_result) exitWith {};
	life_med_calls set [_cur,-1];
	life_med_calls = life_med_calls - [-1];
	publicVariable "life_med_calls";
	_playerslb lbDelete _cur;
	if((lbSize _playerslb) isEqualTo 0) then {
		_playerslb lbAdd (localize "STR_DC_nodispatches");
...
life_call_showcall = compileFinal '
	...
	if(((count life_med_calls)-1) < _lbcur) exitwith {hint localize "STR_DC_reload"};
	_lbcur = life_med_calls select _lbcur;
	hint parseText format ["<t color=""#316dff""><t size=""2""><t align=""center"">" + (localize "STR_DC_title") + "<br/><br/><t color=""#33CC33""><t align=""left""><t size=""1"">" + (localize "STR_DC_msg_to") + ": <t color=""#ffffff"">" + (localize "STR_DC_msg_allmeds") + "<br/><t color=""#33CC33"">" + (localize "STR_DC_msg_by") + ": <t color=""#ffffff"">%1<br/><br/><t color=""#33CC33"">" + (localize "STR_DC_message") + ":<br/><t color=""#ffffff"">%2",(_lbcur select 0),(_lbcur select 2)];
';
life_call_sendmessage = compileFinal '
	...
	if(((count life_med_calls)-1) < _cur) exitwith {hint localize "STR_DC_reload"};
	_to = (life_med_calls select _cur) select 5;
