...
[] Spawn {
    While {True} Do {
	    RJ_IconeFala = RJ_IconeFala + 1;
		If (RJ_IconeFala > 2) Then { RJ_IconeFala = 1; };
		Sleep 0.15;
	};
};
...
While {true} do
{
	...
	{
		sleep 1;
		if (player getVariable["speaking",false]) then {player setVariable["speaking", false, true];};
	...
	{
		if(player getVariable["speaking",false]) then 
		{
			player setVariable["speaking", false, true];
		};
	};
