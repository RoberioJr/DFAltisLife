...
If (RJ_VendeuCasa) ExitWith { Hint "Você Já Vendeu Uma Casa Nessa Sessão, Relogue Para Vender Outra!"; };
...
if (_action) then {
	SystemChat "Iniciando Processo De Venda De Casas, Aguarde...";
	Sleep 1 + round(Random 4);
	If (RJ_VendeuCasa) ExitWith { closeDialog 0; Hint "Você Já Vendeu Uma Casa Nessa Sessão, Relogue Para Vender Outra!"; };
	Sleep 1 + round(Random 2);
	If (RJ_VendeuCasa) ExitWith { closeDialog 0; Hint "Você Já Vendeu Uma Casa Nessa Sessão, Relogue Para Vender Outra!"; };
	RJ_VendeuCasa = true;
	Hint "Casa Vendida Com Sucesso!";
};
...
if (_action) then {
    ...
    [1] call SOCK_fnc_updatePartialRJ;
