...
[] spawn {
			...
			if ((_this select 1) == 33) then {
				if (!mav_tuning_nitroActive) then {
					if ((((vehicle player) getVariable ["mav_tuning",[0,0,0,0,0]]) select 3) > 0) then {
						...
						[format["<t size='1' align='center' shadow='2' font='PuristaMedium' color='#12FE00'>Você Ativou o Nitro, O Veiculo Ficará Mais Rápido Por 60Segundos<br/> Ainda Tem: %1 Cargas Nitro Sobrando</t>",_curValue select 3]] spawn mav_tuning_fnc_displayText;

						[] spawn {
							sleep 60;
							mav_tuning_nitroActive = false;

							["<t size='1' align='center' shadow='2' font='PuristaMedium' color='#12FE00'>O Nitro Expirou</t>"] spawn mav_tuning_fnc_displayText;
						};
					} else {
						["<t size='1' align='center' shadow='2' font='PuristaMedium' color='#ff0000'>Sem Cargas De Nitro Sobrando...</t>"] spawn mav_tuning_fnc_displayText;
					};
				} else {
					["<t size='1' align='center' shadow='2' font='PuristaMedium' color='#ff0000'>O Nitro Já Está Ativo</t>"] spawn mav_tuning_fnc_displayText;
				};
			};
			...
			if ((_this select 2) AND (_this select 1) == 38) then {
				if !((((vehicle player) getVariable ["mav_tuning",[0,0,0,0,[-1,-1,-1]]]) select 4) isEqualTo [-1,-1,-1]) then {
					...
					if (isNull ((vehicle player) getVariable ["mav_tuning_light",objNull])) then {
						...
						["<t size='1' align='center' shadow='2' font='PuristaMedium' color='#12FE00'>Neon Ativado</t>"] spawn mav_tuning_fnc_displayText;
						_handled = true;
					} else {
						[(vehicle player)] remoteExec ["mav_tuning_fnc_disableLight"];
						["<t size='1' align='center' shadow='2' font='PuristaMedium' color='#ff0000'>Neon Desativado</t>"] spawn mav_tuning_fnc_displayText;
						_handled = true;
					};
				};
			};
	...
	// Give all NPCs their addaction
	{
		...
		if (!isNull _npc) then {
			player addAction ["Loja De Tunagem", {[vehicle player] spawn mav_tuning_fnc_start}, nil, 50, true, false, "", format["(%1 distance2d player) < 10", _npcNameString]];
		};
