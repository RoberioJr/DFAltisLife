...
if (vehicle player == player) exitWith {hint "Você Precisa Estar Dentro De Um Veiculo!"};
if (driver vehicle player != player) exitWith {hint "Você Não é O Motorista Desse Veiculo!"};
if ((count crew vehicle player) > 1) exitWith {hint "Você Precisa Estar Sozinho No Veiculo!"};
if (isNil {(vehicle player) getVariable "vehicleID"}) exitWith {hint "Você Precisa Guardar O Veiculo Na Garagem Uma Vez Para Depois Tunar-lo!"};
if (diag_tickTime - mav_tuning_lastVisit < 10) exitWith {hint "Você Tem Que Aguardar Antes De Tunar Novamente!"};
...
// Not tuneable?
if (_configName == "") exitWith {hint "Esse Veiculo Não Pode Ser Tunado!"};
