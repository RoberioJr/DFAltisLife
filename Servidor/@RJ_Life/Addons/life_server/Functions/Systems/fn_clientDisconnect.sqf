...
if (isNil "HC_UID" || {!(_uid isEqualTo HC_UID)}) then {
    ...
    if ((getMarkerPos "respawn_civilian" distance _position) > 300) then {
        ...
        if (life_save_civilian_position && _side isEqualTo civilian) then {
            _alive = alive _unit;
            if (life_HC_isActive) then {[_uid,_side,_alive,4,_position] remoteExec ["HC_fnc_updatePartialRJ",HC_Life];} else {[_uid,_side,_alive,4,_position] spawn DB_fnc_updatePartialRJ;};
        };
    };
};
